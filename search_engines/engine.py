...
class SearchEngine(object):
    ...
    def _extract_emails(self, text):
        '''Extracts and returns emails found in the text.'''
        return re.findall(r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,7}\b', text)

    def _extract_phone_numbers(self, text):
        '''Extracts and returns phone numbers found in the text.'''
        phone_numbers = re.findall(r'\b(?:\+?(\d{1,3})[-.\s]?(\d{1,4})?[-.\s]?(\d{1,4})?[-.\s]?(\d{1,9})\b|\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4})\b', text)
        
        # Flatten the tuples and filter out non-phone numbers
        formatted_numbers = []
        for number in phone_numbers:
            flattened_number = ''.join(filter(None, number))
            
            # Filter out dates and ensure the number is in a valid range
            if len(flattened_number) >= 10 and len(flattened_number) <= 15 and not re.search(r'\b\d{1,2}[-/]\d{1,2}[-/]\d{2,4}\b', flattened_number):
                formatted_numbers.append(flattened_number)
    ...
    def extract_phone_numbers(self):
        ...
        phone_numbers = []
        for row in self.results:
            phone_numbers.extend(row.get('phone_numbers', []))
